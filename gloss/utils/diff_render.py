...
def diff_project_render(mesh, cameras, rendered_views, initial_textures,
                        ...
                        scheduler_step_size=20, scheduler_gamma=0.5, quiet=False, debug=False):
    """
    ...
    resolutions = [int(rendered_views.shape[-2]/4)] + augmented_resolutions
    ...
    for it in range(iterations):
        ...
        for i in range(super_batch_size):
            ...
            resize = torchvision.transforms.Resize((int(batch_cameras[0].height), int(batch_cameras[0].width)))
            ...
            batch_res = fast_batched_render(batch_cameras, mesh, input_textures, backend="cuda")[
