...
# Functions and classes taken from lama other than batched_mask_generate, slight modification to get_mask_generator default kwargs
# Source files: https://github.com/advimman/lama/blob/main/saicinpainting/training/data/masks.py, https://github.com/advimman/lama/blob/main/saicinpainting/utils.py
...
def batched_mask_generate(images, generator=None, full_ratio=0.1, min_ref=1, max_ref=4):
    if generator is None:
        generator = get_mask_generator()
    masks = []
    bz = images.shape[0]
    for i in range(bz):
        mask = 1 - torch.from_numpy(generator(images[i]))
        masks.append(mask.permute(1, 2, 0))
    masks = torch.stack(masks, dim=0).to(images.device)
    full_mask_indices = (torch.rand(bz) < full_ratio).nonzero(as_tuple=True)[0]
    masks[full_mask_indices, ...] = torch.zeros_like(masks[full_mask_indices, ...])
    no_mask_indices = random.sample(range(bz), random.choice(range(min_ref, min(max_ref, bz)+1)))
    masks[no_mask_indices, ...] = torch.ones_like(masks[no_mask_indices, ...])
    return masks
